/*
 * This file is part of the Valkyrja package.
 *
 * (c) Melech Mizrachi <melechmizrachi@gmail.com>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

import { describe, expect, it, vi } from 'vitest';

import { Input } from '../../../../../../src/Valkyrja/Cli/Interaction/Input/Input.ts';
import { Output } from '../../../../../../src/Valkyrja/Cli/Interaction/Output/Output.ts';
import { RouteDispatchedHandler } from '../../../../../../src/Valkyrja/Cli/Middleware/Handler/RouteDispatchedHandler.ts';
import { Route } from '../../../../../../src/Valkyrja/Cli/Routing/Data/Route.ts';
import { Container } from '../../../../../../src/Valkyrja/Container/Manager/Container.ts';

import type { OutputContract } from '../../../../../../src/Valkyrja/Cli/Interaction/Output/Contract/OutputContract.ts';

const route = new Route('build', 'desc', (): OutputContract => new Output());

describe('RouteDispatchedHandler', () => {
    it('passes the output through when there is no middleware', () => {
        const output = new Output();

        expect(new RouteDispatchedHandler(new Container()).routeDispatched(new Input(), output, route)).toBe(output);
    });

    it('delegates to the next middleware', () => {
        const other = new Output();
        const container = new Container();
        container.setSingleton('mw', { routeDispatched: vi.fn(() => other) });

        expect(new RouteDispatchedHandler(container, 'mw').routeDispatched(new Input(), new Output(), route)).toBe(
            other,
        );
    });
});
